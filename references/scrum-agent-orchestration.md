...
*   **Definition of Done (DoD)** *(Phase 5 → 6)*: A task is not complete until 100% of Gherkin ACs pass and test artifacts exist in `tests/results/Sprint-N-BKI-XXX/`.
...
| `tests/results/Sprint-N-BKI-XXX/` | Per-sprint artifacts: `BKI-XXX_unit.txt`, `BKI-XXX_e2e.txt`, `BKI-XXX_ui.png`, `regression_unit.txt`. |